...
def balance_data(X, y, random_state=42):
    """
    Aplica submuestreo para balancear las clases del conjunto de entrenamiento.
    """
    # Combinar X e y
    train_data = X.copy()
...
def train_model(
    ...
    with mlflow.start_run(run_name="DecisionTree_Production"):  # Corregido: mflow → mlflow
        ...
        # Convertir columnas enteras a float en X_test
        int_cols = X_test.select_dtypes(include=['int64', 'int32']).columns
        for col in int_cols:
        ...
        # Calcular métricas
        metrics = {
            "f1_score": float(f1_score(y_test, y_pred)),  # Especificar clase positiva
        ...
        ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=['No', 'Yes']).plot(ax=ax)  # Corregido: faltaba .plot()
        plt.title('Confusion Matrix - Production Model')
        mlflow.log_figure(fig, "confusion_matrix.png")  # Corregido: mflow → mlflow
        plt.close()

        # Registrar pipeline completo
        mlflow.sklearn.log_model(  # Corregido: mflow → mlflow
